...
<body>
 ...
 <div class="logout" ><a href="<%=request.getContextPath()%>/logout">로그아웃</a></div>
	<div class="header">
		<h1>재고 관리</h1>
	</div>
	...
	<div style="display: flex; justify-content: space-between;">
			...
			<table id="item" border="1">
				...
				<tr>
					<td><a href="javascript:void(0);"
						onclick="fillData('<%=g.getGoodsCode()%>', '<%=g.getGoodsName()%>', '<%=g.getGoodsPrice()%>', '<%=g.getGoodsStock()%>')">
							<%=g.getGoodsCode()%>
					</a></td>
					<td><a href="javascript:void(0);"
						onclick="fillData('<%=g.getGoodsCode()%>', '<%=g.getGoodsName()%>', '<%=g.getGoodsPrice()%>', '<%=g.getGoodsStock()%>')">
   	...
   	<%String messageParam = request.getParameter("message");
		String decodedMessage="상품 재고 관리";
		 if (messageParam != null) {           
	            decodedMessage = java.net.URLDecoder.decode(messageParam, "utf-8");
	 } %>
			<p> <%=decodedMessage%></p>
			<form class="edit" action="/MVC/goodsAlter" method="post">
				<!-- 상품코드 입력란에 id 추가 -->
				<div class="input_box">
				...
				<div class="input_box">
					상품코드 <input type="text" name="goodsCode" id="code"
